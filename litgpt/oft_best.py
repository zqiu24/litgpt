...
def forward_core(
    ...
    # x = PermutationFunction.apply(x, perm_in, perm_in_inv)
    x = permute_apply(x, perm_in, perm_in_inv)
    ...
    # y = y_rot.index_select(-1, perm_out_inv)
    # y = PermutationFunction.apply(y_rot, perm_out_inv, perm_out)
    y = permute_apply(y_rot, perm_out_inv, perm_out)
    return y

'''

# Global cache for identity matrices
_EYE_CACHE = {}
...
def cayley_batch(
    ...
    """
    Optimized Cayley parametrization with caching and in-place operations.
    """
    b = Q_skew.shape[0]
    ...
    if use_cayley_neumann:
        ...
        cache_key = (block_size, device, dtype)
        if cache_key not in _EYE_CACHE:
            _EYE_CACHE[cache_key] = torch.eye(block_size, device=device, dtype=dtype)
        
        eye = _EYE_CACHE[cache_key]
        R = eye.expand(b, block_size, block_size).contiguous()
        ...
        if num_neumann_terms > 1:
            ...
            if num_neumann_terms > 2:
                ...
                # Optimized higher-order terms
                if num_neumann_terms > 3:
                    Q_power = Q_squared
                    for i in range(3, num_neumann_terms):
                        Q_power = torch.bmm(Q_power, Q_skew)
                        R.add_(Q_power, alpha=2.0)
    else:
        # Optimized exact Cayley
        cache_key = (block_size, device, dtype)
        if cache_key not in _EYE_CACHE:
            _EYE_CACHE[cache_key] = torch.eye(block_size, device=device, dtype=dtype)
        
        eye = _EYE_CACHE[cache_key].expand(b, block_size, block_size)
        R = torch.linalg.solve(eye + Q_skew, eye - Q_skew, left=False)
...
def get_weight_poet(Rl, Rr, block_size, rows, cols, use_cayley_neumann=True, num_cayley_neumann_terms=5):
    ...
    # Q_skew_cat = skew_symmetric(vec_cat, block_size, rows, cols)
...
@allow_in_graph
def skew_symmetric(vec, block_size, rows, cols):
...
class OFTLinear(OFTLayer):
    ...
    def update_permutation(self):
        ...
        with torch.no_grad():
            ...
            # merge the self.linear.weight with permutations: P_N.t() @ self.linear.weight.t() @ P_M
            W_orig_merged1 = self.linear.weight.index_select(0, self.perm_in)
            W_orig_merged2 = W_orig_merged1.index_select(-1, N_perm_inv)
    ...
    def forward(self, x):
        ...
        key = (
            ...
            tuple(w.shape),
            str(w.dtype),
            tuple(self.oft_L.shape) if hasattr(self, "oft_L") else (),
            tuple(self.oft_R.shape) if hasattr(self, "oft_R") else (),
        ...
        out = fn(
            x, self.oft_L, self.oft_R, self.oft_block_size,
            self.rows, self.cols,
...
class OFTQKVLinear(OFTLinear):
    ...
    def merge_then_reinitialize(self) -> None:
        with torch.no_grad():
            R_left, R_right = get_weight_poet(self.oft_L, self.oft_R, self.oft_block_size, self.rows, self.cols)
            R_left = torch.block_diag(*R_left)
            R_right = torch.block_diag(*R_right)

            P_N = torch.eye(self.in_features, device=R_right.device, dtype=R_right.dtype)[self.perm_in]
            P_M = torch.eye(self.out_features, device=R_right.device, dtype=R_right.dtype)[self.perm_out]

            # orig_weights = base_layer.weight.data.to(R_right_bs.dtype)
            # orig_weights = torch.transpose(orig_weights, 0, 1)
            orig_weights = P_N @ R_right_bs @ P_N.t() @ orig_weights.t() @ P_M @ R_left_bs @ P_M.t()
            # orig_weights = torch.transpose(orig_weights, 0, 1)
            orig_weights = orig_weights.t()

            base_layer.weight.data = orig_weights.contiguous().to(orig_dtype)

            self.linear.weight.data.copy_(self.oft_R.get_weight())
            self.oft_R.zero_()
            self.oft_L.zero_()
